...
impl MyUplinkClient {
    ...
    > {
        Box::pin(async move {
            ...
            if status.as_u16() == 401 {
                warn!(
                    "Received 401 Unauthorized from {}. Invalidating token and retrying...",
                    url
                );
                if !retry {
            ...
            if status.as_u16() == 429 {
                ...
                warn!(
                    "Rate limited (429) from {}. Retry-After: {:?}",
                    url, retry_after
                );
    ...
    async fn fetch_device_points(
        &self,
        token: &str,
        device_id: &str,
    ) -> Result<Vec<Parameter>, MyUplinkError> {
        ...
        let response = self
            ...
            .map_err(|e| {
                error!(
                    "Network error fetching device points for {}: {}",
                    device_id, e
                );
                MyUplinkError::Network(e.to_string())
            })?;
        ...
        let status = response.status();
        debug!(
            "Device points response status: {} for device {}",
            status.as_u16(),
            device_id
        );
        ...
        if !status.is_success() {
            error!(
                "HTTP {} error fetching device points for {} from {}",
                status.as_u16(),
                device_id,
                url
            );
        ...
        let points: Vec<DevicePoint> = response.json().await.map_err(|e| {
            error!(
                "Failed to parse device points response for {}: {}",
                device_id, e
            );
            MyUplinkError::ParseError(e.to_string())
        })?;
                ...
                point.value.and_then(|v| {
                    if let Some(num) = serde_json::Number::from_f64(v) {
                        debug!(
                            "Including parameter {}: {} = {}",
                            point.parameter_id,
                            point.parameter_name.as_deref().unwrap_or("(no name)"),
                            v
                        );
                        Some(Parameter {
                        ...
                        warn!(
                            "Failed to convert value {} to JSON number for parameter {}",
                            v, point.parameter_id
                        );
                        None
                    }
                })
        ...
        debug!(
            "Converted {} device points to parameters for device {}",
            parameters.len(),
            device_id
        );
        Ok(parameters)
